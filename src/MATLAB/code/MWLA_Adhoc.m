function [decision, frame_next] = MWLA_Adhoc(nPhase, feasible_phase, current_location, current_jobs, memory_jobs, frame_remained, average_rate, beta, IDLE_ON, IDLE_MAX, power)
...
% pressure: pressure matrix of dimension nLink-by-1
    ...
    for i=1:nPhase
        ...
        frameSize(i) = (leading_factor*((sum(memory_jobs))^power));
        if i == current_location 
            ...
            %weight(i) = (frameSize(i) + 1)*(sum(current_jobs(feasible_phase{i}).*beta(feasible_phase{i})./average_rate(feasible_phase{i})));
        else
