...
%power = 0.65;
%power = [0 0.01 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4];
%power = [0 0.01 0.2];
%power = [0.2 0.55 0.6];
%power2 = 0.9;
%power2 = [0.6 0.7 0.8 0.9];
%power = 0.5;
%power = [0 0.001 0.5 0.7];
%power = 0.99;
power = 0.001;
simT = 200000;
run = 1;
...
fileID = fopen('swSingleHop.log','a');
%rho = 0.75;
%rho = [0.9 0.91 0.92 0.93 0.94 0.95 0.96 0.97 0.98 0.99];
%rho = 0.9;
...
for i=1:size(policy,1)
    ...
    swSingleHop(setup, simT, run, policy{i}, power(i), fileID, logMode, rho); 
end
%}
...
setup = 22;
fprintf(fileID, 'Scheduling-Policy: %s\n', policy);
fprintf(fileID, 'setup-number: %d  simT: %d  iterations: %d  rho: %.2f\n', setup, simT, run, rho);
for i=1:length(Ts)
...
fprintf(fileID, 'setup-number: %d  simT: %d  iterations: %d\n', setup, simT, run);
for i=1:length(power)
    fprintf(fileID, 'power = %.3f ', power(i));
    swSingleHop_0709(setup, simT, run, policy, power(i), fileID, logMode, rho, Ts); 
end
%}
%{
