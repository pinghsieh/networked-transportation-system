function [decision, frame_next] = VFMW_Adhoc_ext(nPhase, feasible_phase, current_location, current_jobs, frame_remained, average_rate, beta_matrix)
...
% pressure: pressure matrix of dimension nLink-by-1
    ...
    for i=1:nPhase
        ...
        weight(i) = sum((current_jobs(feasible_phase{i}).*average_rate(feasible_phase{i})).*(beta_matrix{i}));
    end
