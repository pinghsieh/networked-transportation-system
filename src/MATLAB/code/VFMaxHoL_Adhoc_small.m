function [decision, frame_next] = VFMaxHoL_Adhoc_small(nPhase, feasible_phase, current_location, current_jobs, frame_remained, age, lambda, IDLE_ON, IDLE_MAX)
...
% Use small alpha
% Switchover rule: next weight > (current weight + constant)
    power = 0.2;
    %offset = 0;
    %power2 = 0.799;
    leading_factor = 1;
    ...
    if current_location > 0
        % Debugging
        %offset = (max(weight))^(power2);
        %weight(current_location) = weight(current_location) + offset;
        weight(current_location) = weight(current_location) + IDLE_MAX*sum(age((feasible_phase{current_location}))); 
