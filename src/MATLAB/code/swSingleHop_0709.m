function [] = swSingleHop_0709(arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9)
...
if arg1 == 42
    nLink = 6; 
    nPhase = 4;
    feasible_phase{1} = [1 3 5 6];  % 30% 
    feasible_phase{2} = [1 4 5 6];  % 30%
    feasible_phase{3} = [2 3 5 6];  % 20%
    feasible_phase{4} = [2 4 5 6];  % 20%
    prob_arrival = arg8*[0.48; 0.28; 0.25; 0.2; 0.9; 0.6];
    beta = [1; 1; 1; 1; 1; 1];
    lambda = [1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
    %prob_on_channel = [1; 1; 1; 1; 1; 1];
    prob_on_channel = [0.8; 0.7; 0.5; 0.4; 0.9; 0.6];
    init_jobs = [0; 0; 0; 0; 0; 0];
end
% Setting #43
if arg1 == 43
    ...
    feasible_phase{3} = [2 3 5 6];  % 20%
    feasible_phase{4} = [2 4 5 6];  % 20%
    prob_arrival = arg8*[0.18; 0.16; 0.25; 0.3; 0.9; 0.8];
    beta = [1; 1; 1; 1; 1; 1];
    lambda = [1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
    %prob_on_channel = [1; 1; 1; 1; 1; 1];
    prob_on_channel = [0.3; 0.4; 0.5; 0.6; 0.9; 0.8];
    init_jobs = [0; 0; 0; 0; 0; 0];
end
% Setting #44
if arg1 == 44
    ...
    feasible_phase{1} = [1 3 5 6];  % 30% 
    feasible_phase{2} = [1 4 5 6];  % 40%
    feasible_phase{3} = [2 3 5 6];  % 10%
    feasible_phase{4} = [2 4 5 6];  % 20%
    prob_arrival = arg8*[0.49; 0.06; 0.36; 0.36; 0.8; 0.5];
    beta = [1; 1; 1; 1; 1; 1];
    lambda = [1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
    %prob_on_channel = [1; 1; 1; 1; 1; 1];
    prob_on_channel = [0.7; 0.2; 0.9; 0.6; 0.8; 0.5];
    init_jobs = [0; 0; 0; 0; 0; 0];
end
% Setting #45
if arg1 == 45    
    ...
    feasible_phase{5} = [5 6]; 
    feasible_phase{6} = [7 8];
    prob_arrival = arg8*[0.1; 0.5; 0.1; 0.3; 0.1; 0.5; 0.1; 0.3];
    beta = [1; 1; 1; 1; 1; 1; 1; 1];
    lambda = [1; 1; 1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
    prob_on_channel = [1; 1; 1; 1; 1; 1; 1; 1];
    init_jobs = [0; 0; 0; 0; 0; 0; 0; 0];
...
if arg1 == 46    
    ...
    feasible_phase{5} = [5 6]; 
    feasible_phase{6} = [7 8];
    prob_arrival = arg8*[0.02; 0.26; 0.24; 0.48; 0.24; 0.48; 0.02; 0.26];
    beta = [1; 1; 1; 1; 1; 1; 1; 1];
    lambda = [1; 1; 1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
    prob_on_channel = [1; 1; 1; 1; 1; 1; 1; 1];
    init_jobs = [0; 0; 0; 0; 0; 0; 0; 0];
...
if arg1 == 47
    ...
    feasible_phase{3} = 3; 
    feasible_phase{4} = 4; 
    prob_arrival = arg8*[0.125; 0.125; 0.125; 0.125];
    beta = [1; 1; 1; 1];
    lambda = [1; 1; 1; 1]; % For max-HoL-age policy
    %prob_on_channel = [1; 1; 1; 1];
    prob_on_channel = [0.5; 0.5; 0.5; 0.5];
...
history_cumulative_real_service = zeros(nLink, simT);
cumulative_arrival_tracker = zeros(nLink, simT);
    ...
    for t = 1:simT
        ...
        switch arg4
            ...
            case 'MWLAHOQ_Adhoc'
                ...
                arg_policy = {nPhase, feasible_phase, server_location, HoL_age, memory_HoL, frame_remained, prob_on_channel, beta, IDLE_ON, IDLE_MAX, arg5};
            otherwise
                policy = @VFMW_Adhoc;
        ...
        if idle == 0
            if decision > 0
                if (server_location ~= decision) && (IDLE_ON == 1)
                    ...
                    memory_HoL = HoL_age; % New: for MW-LA-HOQ policy
                else
        ...
        cumulative_real_service = cumulative_arrival - current_jobs;
        history_cumulative_real_service(:, t) = cumulative_real_service;
        history_jobs(:,t) = current_jobs; 
        current_debt = current_debt + arrival_in_this_slot;
        history_debt(:,t) = current_debt; 
        if t > 1
            cumulative_arrival_tracker(:,t) = cumulative_arrival_tracker(:,t-1) + arrival_in_this_slot;
        else
            cumulative_arrival_tracker(:,t) = arrival_in_this_slot;
        end
        
        % Update HoL timestamp
        for k=1:nLink
            [M Id] = max(cumulative_arrival_tracker(k, HoL_timestamp(k):t) > cumulative_real_service(k));
            if M == 0
                HoL_timestamp(k) = t + 1; 
            else
                HoL_timestamp(k) = HoL_timestamp(k) + Id - 1;
            end
        end
...
% 
    ...
    history_jobs_phase_average = history_jobs_phase_average/iteration;
...
%% Debugging
    %%createfigure4(time, history_jobs_average);  
    ...
    %%createfigure4(time, cumulative_arrival_tracker);
    %%createfigure4(time, history_cumulative_real_service);
    ...
    if flog == 1
        ...
        if strcmp(arg7, 'foreach')
            for i=1:nLink
                fprintf(fileID, 'Client-%d mean-queue-length %.1f \n', i, history_mean_jobs_average(i, simT));
            end             
        end
