...
%prob_arrival = [0.121; 0.121; 0.121; 0.121];
prob_arrival = [0.12; 0.12; 0.12; 0.12];
%prob_arrival = [0.25; 0.125; 0.0625; 0.05];
%prob_arrival = [0.5; 0.25; 0.125; 0.1];
%prob_arrival = [0.3; 0.3; 0.15; 0.15];
%prob_arrival = [0.5; 0.3; 0.3; 0.05];
%prob_arrival = [0.4; 0.3; 0.15; 0.05];
%prob_arrival = [0.4; 0.3; 0.15; 0.05875];
...
%history_mean_HoL_age2_average = zeros(nLink, simT);
...
for i = 1:iteration
    ...
    %history_mean_HoL_age2 = zeros(nLink, simT);
    ...
    for t = 1:simT
        ...
        %[decision, frame_remained] = VFMW_non_idling(server_location, current_jobs, frame_remained, prob_on_channel, beta);
        %[decision, frame_remained] = maxHoLage_Frame(server_location, current_jobs, frame_remained, prob_on_channel, HoL_age, lambda);
        %decision = maxHoLage_Ex(server_location, current_jobs, prob_on_channel, HoL_age, lambda);
    ...
    %history_mean_HoL_age2_average = history_mean_HoL_age2_average + history_mean_HoL_age2;
end
