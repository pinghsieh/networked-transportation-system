...
feasible_phase{2} = [2 3]; 
%prob_arrival = [0.1; 0.98; 0.1];
arrival_det = [60; 10; 36];
beta = [1; 1; 1];
lambda = [1; 1; 1]; % For max-HoL-age policy
%prob_on_channel = [1; 1; 1];
service_det = [100; 100; 100];
init_jobs = [0; 0; 0];
%}
% Setting #20
%{
...
feasible_phase{3} = [3 4]; 
%feasible_phase{4} = [1 4]; 
arrival_det = [40; 4; 1; 20];
beta = [1; 1; 1; 1];
lambda = [1; 1; 1; 1]; % For max-HoL-age policy
service_det = [100; 100; 100; 100];
init_jobs = [0; 0; 0; 0];
%}
% Setting #21
%{
nLink = 8; 
nPhase = 6;
feasible_phase{1} = [1 2]; % 2
feasible_phase{2} = [2 6]; % 50
feasible_phase{3} = [3 4]; % 20
feasible_phase{4} = [4 8]; % 5
feasible_phase{5} = [5 6]; % 3
feasible_phase{6} = [7 8]; % 5
arrival_det = [2; 32; 20; 25; 3; 33; 5; 10];
beta = [1; 1; 1; 1; 1; 1; 1; 1];
%beta = [5; 1; 5; 2.5; 5; 1; 5; 2.5];
lambda = [1; 1; 1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
service_det = [100; 100; 100; 100; 100; 100; 100; 100];
init_jobs = [0; 0; 0; 0; 0; 0; 0; 0];
%}
% Setting #22
%{
nLink = 4; 
nPhase = 3;
feasible_phase{1} = [1 2];  % 3
feasible_phase{2} = [1 3];  % 3
feasible_phase{3} = [2 4];  % 3
%feasible_phase{4} = [2 4];  % 90
arrival_det = [104; 10; 250; 80];
beta = [1; 1; 1; 1];
lambda = [1; 1; 1; 1]; % For max-HoL-age policy
service_det = [200; 100; 500; 1000];
init_jobs = [0; 0; 0; 0];
%}
% Setting #23
%{
nLink = 5; 
nPhase = 4;
feasible_phase{1} = [1 2 3];  % 20
feasible_phase{2} = [1 2 4];  % 1
feasible_phase{3} = [3 4 5];  % 30
feasible_phase{4} = [2 5];  % 9
arrival_det =[31.5; 45; 75; 46.5; 58.5];
beta = [1; 1; 1; 1; 1];
lambda = [1; 1; 1; 1; 1]; % For max-HoL-age policy
service_det = [100; 100; 100; 100; 100];
init_jobs = [0; 0; 0; 0; 0];
%}
% Setting #24
%{
nLink = 3; 
nPhase = 3;
feasible_phase{1} = [1 2];  % 32
feasible_phase{2} = [1 3];  % 32
feasible_phase{3} = [2 3];  % 32
arrival_det =[64; 64; 64];
beta = [1; 1; 1];
lambda = [1; 1; 1]; % For max-HoL-age policy
service_det = [100; 100; 100];
init_jobs = [0; 0; 0];
...
% Setting #26 (Celik's simulation test case)
%{
nLink = 3; 
nPhase = 3;
feasible_phase{1} = [1 2];   
feasible_phase{2} = [1 3];  
feasible_phase{3} = [2 3];  
arrival_det =[20; 70; 60];
beta = [1; 1; 1];
lambda = [1; 1; 1]; % For max-HoL-age policy
service_det = [100; 100; 100];
init_jobs = [0; 0; 0];
...
history_TSLSW_average = zeros(1, simT);
...
for i = 1:iteration
    ...
    for t = 1:simT
        ...
        current_service_rate = service_det; % Use deterministic service
        ...
        [decision, frame_remained] = VFMW_Adhoc_diff(nPhase, feasible_phase, server_location, current_jobs, frame_remained, service_det, beta, IDLE_ON, IDLE_MAX);
        %[decision, frame_remained] = VFMW_Adhoc_empF(nPhase, feasible_phase, server_location, current_jobs, frame_remained, service_det, beta, IDLE_ON, IDLE_MAX, TSLSW);
        ...
        %[decision, frame_remained] = MaxWeight_Adhoc(nPhase, feasible_phase, server_location, current_jobs, frame_remained, service_det, beta);
                
        % New arrivals
        %arrival_in_this_slot = (prob_arrival >= (unifrnd(0, 1, nLink, 1)));
        arrival_in_this_slot = arrival_det; % Use deterministic arrivals
        ...
        if idle == 0
            if decision > 0
                if (server_location ~= decision) && (IDLE_ON == 1)
                    ...
                    % Update time since last switchover
                    TSLSW = TSLSW + 1;
                end
    ...
    history_TSLSW_average = history_TSLSW_average + history_TSLSW;
end
...
% 
    ...
    history_TSLSW_average = history_TSLSW_average/iteration;
    %for h=1:nPhase
...
%% Debugging
    ...
    %createfigure(time, diag(1./service_det)*history_jobs_average);
    createfigure(time, diag(1./(service_det-arrival_det))*history_jobs_average);
    %createfigure_single(time, history_location);  
    %createfigure_single(time, history_TSLSW); 
