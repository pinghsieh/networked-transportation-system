function network_adhoc_func(load, policy)
...
feasible_phase{4} = [4 8]; 
prob_arrival = load*[0.125; 0.375; 0.125; 0.375; 0.125; 0.375; 0.125; 0.375];
beta = [1; 1; 1; 1; 1; 1; 1; 1];
lambda = [1; 1; 1; 1; 1; 1; 1; 1]; % For max-HoL-age policy
prob_on_channel = [1; 1; 1; 1; 1; 1; 1; 1];
init_jobs = [0; 0; 0; 0; 0; 0; 0; 0];
...
%% Debugging
    ...
    fpath = sprintf('./delay_%s.dat', policy);
    fileID = fopen(fpath,'a');
    fprintf(fileID, '%.3f %.2f\n', load, sum(history_mean_jobs_average(:,simT))/sum(prob_arrival));
    for i=1:nLink
