function func_smart_networks(simT, amber_length, allred_length, arrival_rate_scaling, policy, alpha, beta_1, beta_2)
%%------------------------------------------------------ 
...
%arrival_rate_scaling = 2.4; % arrival rate scaling factor
...
for t=1:simT
    ...
    for m=1:N_intersections
        ...
        if switchover_state(m) == 0
            ...
            service_count{m}(scheduled_phase_id) = service_count{m}(scheduled_phase_id) + 1;
        ...
        if m == 1
            ...
            fprintf(QL_Log_Node1_ID, '\t %d\t %d', current_phase(1), switchover_state(1));  
        end
    end  
